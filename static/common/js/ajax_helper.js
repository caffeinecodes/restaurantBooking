...
var ajaxFactory = {
                ...
                if (xhr.upload) {
                    xhr.upload.addEventListener('progress', function (evt) {
                        if(evt.lengthComputable){
                            var percent = (evt.loaded*.75 / evt.total) * 100;
                            progress_bar_card.find('.progress-bar').width(percent+'%');
                            progress_bar_card.find('.uploaded_file_size').text(parseFloat(progress_bar_card.find('.total_file_size').text()*evt.loaded/evt.total).toFixed(1));
    ...
    fake_progress: function (progress_bar_card) {
        temp_interval = setInterval(function() {
            if(!(progress_bar_card.find('.progress-section').hasClass('error-sect')|| progress_bar_card.find('.progress-section').hasClass('completed'))){
                var curr_width_in_percent = progress_bar_card.find('.progress-bar').width()/ progress_bar_card.find('.progress-bar').parent().width()*100;
                var random_no = Math.random() * 2;
                var new_width = curr_width_in_percent+random_no;
                if(new_width <= 99){
                    progress_bar_card.find('.progress-bar').width(new_width+'%');
                }else{
    ...
    secureHTTPRequestHandler: function (url, method, data, callback, type) {
        ...
        if (token == '' || type == 'expired') {
            ajaxFactory.ajaxHandler(base_url + '/api/token/', 'GET', {}, function (response) {
