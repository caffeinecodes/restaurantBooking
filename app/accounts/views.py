...
def confirm_email(request, key):
    ...
    msg = EmailMultiAlternatives(
        ...
        body=message,
        from_email="admin@commonproject.com",
        to=[user.email])
...
def send_verification_email(request):
    ...
    try:
        ...
        return Response({
            ...
            'error',
            'message':
            'Email id is not registered. Please enter the valid email id.'
        })
...
def forgot_password(request):
    if request.method == 'POST':
        ...
        activation_url = "{1}/accounts/update_password/{0}".format(
            activation_key, os.environ.get('HOST_NAME'))

        rendertxt = render_to_string('email/reset_password.html', {
            'user': user,
